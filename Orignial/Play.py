...
def play_split_hands(hand1, hand2, dealer, strategy, original_bet, player, count_system = None):
    ...
    if count_system:
        player.update_count(count_system(card_tracker))
...
def play_single_game(deck: Deck, count_system = None, bet_strategy = zero_memory, strategy=basic, verbose=False, starting_bankroll=200):
    """
    Play through a single game with the given strategy.

    Args:
        deck: The deck or shoe to play with
        strategy: The playing strategy function to use
        count_system: counting system used
    ...
    while not dealer.is_empty():
        ...
        try:
            ...
            if dealer.has_blackjack():
                ...
                if count_system:
                    player.update_count(count_system(player.get_hand() + dealer.get_hand()))
                player.reset()
                dealer.reset()
                continue
            ...
            if verbose:
                ...
                print(f"Current Player Count: {player.count}")
...
if __name__ == "__main__":
    ...
    win_rate, profit_loss, games, total_wagered = play_single_game(
        deck, strategy=basic, verbose=True)
